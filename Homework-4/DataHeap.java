...
class DataHeap extends DataBT implements IHeap {
    ...
    /**
     * The new bin tree is checked for all the elements of the original bin tree.
     * @param bt creates a new bin tree
     * @return true if the new bin tree is the same as the original bin tree.
     */
    public boolean containedInBT(IBinTree bt) {
        if (bt.hasElt(this.data)) {
            if (this.left.containedInBT(bt) && this.right.containedInBT(bt))
                return true;
            else
                return false;
        } else
